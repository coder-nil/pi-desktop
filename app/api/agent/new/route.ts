...
import { expireOtherCheckpoints } from "@/lib/checkpoint";
...
export async function POST(req: Request) {
  ...
  try {
    ...
    const { provider, modelId, toolNames, thinkingLevel, uiLocale, chatMode, ...promptCommand } = command as { provider?: string; modelId?: string; toolNames?: string[]; thinkingLevel?: unknown; uiLocale?: unknown; chatMode?: unknown; [key: string]: unknown };
    ...
    const { session, realSessionId } = await startRpcSession(tempKey, "", cwd, {
      ...(toolNames ? { toolNames } : {}),
      ...(chatMode === true ? { chatMode: true } : {}),
      ...(provider && modelId ? { initialModel: { provider, modelId } } : {}),
      ...(explicitThinkingLevel ? { thinkingLevel: explicitThinkingLevel } : {}),
      uiLocale: uiLocale === "zh-CN" ? "zh-CN" : "en",
    });

    await expireOtherCheckpoints(realSessionId);
