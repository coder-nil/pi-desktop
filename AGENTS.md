...
- **The 常规 page is a grouped list, not a table of hairline rows.** Rows live in `SettingsGroup` sections and are styled by the `.settings-general` block in `app/globals.css` (there are no per-row inline styles to audit any more): no per-row `border-bottom`, rows separate through hover highlight plus whitespace, and only the two groups that really need a label (聊天, Shell 工具) carry a group title. 显示横幅 sits with 主题/语言 at the top; putting it between 聊天 and Shell 工具 made it read as a chat setting.
- Visual consistency rules for that page, all pinned by `components/SettingsPanel.test.mjs`: icons come from **lucide-react only** (the repo's single icon dependency, `strokeWidth` 1.8, sizes 17 inside rows and 14 inside row buttons) and no hand-rolled `svg` path is allowed; the radius scale is 10 for the dialog, 8 for rows/icons/controls and pill for the switch track; all interactive controls share one `:focus-visible` ring; `@media (prefers-reduced-motion: reduce)` inside `.settings-general` disables the transitions.
- **The switch's off track is `--switch-track-off`, never `--border`.** `--border` on white is a 1.3:1 boundary, below the 3:1 WCAG 1.4.11 asks of a UI component, which is why the off state is a mid grey (`#8f8f8f` light, `#6b6b6b` dark). Group titles and row descriptions use `--text-muted` for the same reason (`--text-dim` is only for disabled text).
...
`package.json.version` is the only application version source. Bump it with `npm run version:next` — that derives the next version (`0.85.1-alpha.13` → `0.85.1-alpha.14`, `1.2.3` → `1.2.4`, `1.2.3-beta` → `1.2.3-beta.1`), synchronizes `package-lock.json`, `src-tauri/tauri.conf.json`, `src-tauri/Cargo.toml`, `src-tauri/Cargo.lock`, and the README UI version, inserts a `## [<version>] - <date>` CHANGELOG section with the standard version-unification note when that version has none, then re-runs the check pass. Use `npm run version:set -- <version>` when the version is not a simple increment. Existing CHANGELOG entries are historical and are never rewritten; fill the new section with release notes by hand. `npm run version:check` remains the CI entry (it also compares the release tag against the same source). Never hand-edit those derived version fields or hardcode the application version in runtime code; `next.config.ts` exposes it through `NEXT_PUBLIC_PACKAGE_VERSION`.
...
- Never let the public scheme reach the loopback upstream. Cloudflare injects `X-Forwarded-Proto: https` (plus `X-Forwarded-Host`), and Next builds `request.url` from `X-Forwarded-Proto` — so a request the proxy had already rewritten to `Host: 127.0.0.1:<nextPort>` / `Origin: http://127.0.0.1:<nextPort>` is seen by `proxy.ts` as `https://127.0.0.1:<nextPort>` and fails the same-origin comparison. `rewrite_request_head()` therefore strips both headers (`drops_forwarded_scheme_headers_so_the_loopback_origin_still_matches`). The symptom is narrow and misleading: only requests carrying an `Origin` header are rejected — i.e. writes, like sending a prompt from the phone (`POST /api/agent/[id]` → “Untrusted API request”) — while `/m`, `/api/mobile/state` and the SSE stream keep working, because same-origin GETs send no `Origin`.
