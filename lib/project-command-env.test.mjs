...
test("agent bash tells the model to use the Desktop credential prompt", async () => {
  const extension = createProjectCommandBashExtension({
    cwd: process.cwd(),
    settings: {
      getShellCommandPrefix: () => undefined,
...
test("project commands add current-user exports and prioritize the user PATH", async () => {
  const environment = await captureOperationEnvironment({
    ...
    currentUserEnvironment: {
      HOME: "/home/from-shell",
      PATH: "/home/current/.local/bin:/opt/homebrew/bin:/usr/bin",
      USER_EXPORTED_SETTING: "loaded",
      PI_SESSION_ID: "stale-shell-session",
    },
    platform: "linux",
  });
  ...
  assert.deepEqual(environment, {
    HOME: "/home/current",
    PATH: "/agent/bin:/home/current/.local/bin:/opt/homebrew/bin:/usr/bin:/desktop/bin",
    USER_EXPORTED_SETTING: "loaded",
    PI_SESSION_ID: "session-current",
  });
});

test("a user extension keeps priority over the Pi Desktop fallback bash tool", async () => {
  const userBash = {
    name: "user-bash",
    factory: (pi) => {
      pi.registerTool({
