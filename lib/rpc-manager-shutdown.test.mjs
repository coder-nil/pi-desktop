...
function makePromptInner(prompt) {
  return {
    ...
    settingsManager: { setProjectTrusted: () => {}, applyOverrides: () => {} },
    agent: { state: {}, streamFunction: () => {} },
...
test("new prompts reload local resources before building the next agent run", async (t) => {
  const calls = [];
  ...
  inner.reload = async () => calls.push("reload");
  const wrapper = new AgentSessionWrapper(inner);
  t.after(() => wrapper.destroy());

  await wrapper.send({ type: "prompt", message: "use current skills" });
  await nextTurn();
  await wrapper.send({
    type: "prompt",
  ...
  assert.deepEqual(calls, ["reload", "prompt", "prompt"]);
});
...
test("get_state waits for extension resources before returning the system prompt", async (t) => {
  ...
  // The value must come from the post-binding state, not the pre-binding one. Pi
  // Desktop appends the UI-language instruction to the effective prompt, so assert
  // on the base text rather than on the full string (see effectiveSystemPrompt()).
  assert.match(state.systemPrompt, /^after extensions/);
  assert.doesNotMatch(state.systemPrompt, /before extensions/);
});
...
test("accepted prompt failures still finish through the event stream", async (t) => {
  ...
  assert.equal(logged.some((line) => line.includes('"event":"prompt_failed"')), true);
  assert.equal(logged.some((line) => line.includes('"accepted":true')), true);
  assert.equal(logged.some((line) => line.includes("post-accept failure")), true);
});

test("provider message errors are written to structured diagnostics without message content", (t) => {
  const originalConsoleError = console.error;
  const logged = [];
  t.after(() => {
    console.error = originalConsoleError;
  ...
  Object.assign(inner, {
    ...
    model: { provider: "Coding", id: "gpt-5.6-sol" },
    subscribe(next) {
      listener = next;
      return () => {};
    },
  ...
  const line = logged.find((entry) => entry.includes('"event":"provider_message_error"'));
  assert.ok(line);
  assert.match(line, /Anthropic stream ended before message_stop/);
  assert.match(line, /"responseId":"msg_test"/);
  assert.match(line, /"contentTypes":\["thinking","toolCall"\]/);
  assert.match(line, /"toolNames":\["bash"\]/);
  assert.doesNotMatch(line, /private reasoning|secret command/);
});
