...
test("clearing the last entry removes the storage key entirely", () => {
  ...
  clearLastOpen("root-a", storage);
  assert.equal(getLastOpenSession("root-a", storage), null);
  assert.equal(storage.values.has("pi-desktop:last-open-by-workspace"), false);
});

test("ignores a corrupt stored map", () => {
  const storage = createStorage({ "pi-desktop:last-open-by-workspace": "{not-json" });
  assert.equal(getLastOpenSession("root-a", storage), null);
});

test("ignores a stored map of the wrong shape", () => {
  const storage = createStorage({ "pi-desktop:last-open-by-workspace": `"just a string"` });
  assert.equal(getLastOpenSession("root-a", storage), null);
});

test("recovers from an array-shaped stored map", () => {
  const storage = createStorage({ "pi-desktop:last-open-by-workspace": "[]" });
  setLastOpenSession("root-a", "session-a", storage);
  assert.equal(getLastOpenSession("root-a", storage), "session-a");
});
...
test("ignores an empty or non-string session id", () => {
  const storage = createStorage({
    "pi-desktop:last-open-by-workspace": `{"root-a": "", "root-b": 42, "root-c": "ok"}`,
  });
  assert.equal(getLastOpenSession("root-a", storage), null);
...
test("last selected project round-trips and can be cleared", () => {
  const storage = createStorage();
  setLastSelectedProject({ key: "project:a", root: "/repos/a" }, storage);
  assert.deepEqual(getLastSelectedProject(storage), { key: "project:a", root: "/repos/a" });
  clearLastSelectedProject(storage);
  assert.equal(getLastSelectedProject(storage), null);
});

test("last selected project ignores malformed stored values", () => {
  const storage = createStorage({
    "pi-desktop:last-selected-project": JSON.stringify({ key: "project:a", root: 42 }),
  });
  assert.equal(getLastSelectedProject(storage), null);
});

test("last selected project storage failures are ignored", () => {
  const unavailable = {
    getItem() { throw new Error("blocked"); },
    setItem() { throw new Error("blocked"); },
    removeItem() { throw new Error("blocked"); },
  };
  assert.equal(getLastSelectedProject(unavailable), null);
  assert.doesNotThrow(() => setLastSelectedProject({ key: "project:a", root: "/repos/a" }, unavailable));
  assert.doesNotThrow(() => clearLastSelectedProject(unavailable));
});
