import type { LocalePlugin } from "../types";

/** Pi Desktop 内置英语语言包。 */
export const enLocale: LocalePlugin = {
  id: "en",
  label: "English",
  messages: {
    "common.ok": "OK",
    "common.language": "Language",
    "common.models": "Models",
    "common.skills": "Skills",
    "common.plugins": "Plugins",
    "common.settings": "Settings",
    "common.git": "Git",
    "settings.title": "Settings",
    "settings.close": "Close settings",
    "settings.back": "Back to settings",
    "settings.general": "General",
    "settings.generalDescription": "Theme, language, chat and interface preferences",
    "settings.groupChat": "Chat",
    "settings.thinkingExpandedDefault": "Expand thinking by default",
    "settings.thinkingExpandedDefaultDescription": "Show thinking content without expanding each block",
    "settings.chatContentWidth": "Chat content width",
    "settings.chatContentWidthDescription": "Maximum width of messages and the composer",
    "settings.chatContentFontSize": "Chat content font size",
    "settings.chatContentFontSizeDescription": "Text size of messages and the composer",
    "settings.resetChatContentWidth": "Reset width",
    "settings.resetChatContentFontSize": "Reset font size",
    "settings.shellTool": "Shell tool",
    "settings.shellToolDescription": "Run commands through the PowerShell tool on Windows",
    "settings.usePowerShell": "Use PowerShell",
    "settings.about": "About Pi Desktop",
    "settings.aboutDescription": "v{version} · pi v{piVersion}",
    "settings.shortcuts": "Keyboard shortcuts",
    "settings.shortcutsDescription": "See the keyboard actions available in Pi Desktop",
    "settings.shortcutsReadOnly": "Shortcuts are shown for reference and cannot currently be customized.",
    "settings.shortcutsOr": "or",
    "settings.shortcutsGroupGlobal": "Global",
    "settings.shortcutsGroupComposer": "Composer",
    "settings.shortcutsGroupStreaming": "While responding",
    "settings.shortcutsGroupMenus": "Suggestion menus",
    "settings.shortcutsGroupFiles": "File viewer",
    "settings.shortcutNewSession": "Start a new session",
    "settings.shortcutShowSidebar": "Show the sidebar",
    "settings.shortcutToggleConsole": "Toggle the console",
    "settings.shortcutOpenDirectory": "Open a directory",
    "settings.shortcutSendMessage": "Send the message",
    "settings.shortcutSendMessageNote": "Enter on desktop; Ctrl/Cmd + Enter on mobile",
    "settings.shortcutNewLine": "Insert a new line",
    "settings.shortcutInputHistory": "Open input history from an empty composer",
    "settings.shortcutRemoveTag": "Remove the nearest mention or command tag",
    "settings.shortcutStopResponse": "Stop the current response",
    "settings.shortcutSteerResponse": "Send as a steering message",
    "settings.shortcutQueueFollowup": "Queue a follow-up message",
    "settings.shortcutMoveSelection": "Move through suggestions",
    "settings.shortcutMoveSlashGrid": "Move across slash-command columns",
    "settings.shortcutChooseSuggestion": "Choose the selected suggestion",
    "settings.shortcutCloseMenu": "Close the suggestion menu",
    "settings.shortcutMentionLines": "Insert selected source lines into the composer",
    "settings.modelsDescription": "Providers, credentials, and model defaults",
    "settings.skillsDescription": "Manage loaded skills for this project",
    "settings.pluginsDescription": "Discover and manage Pi extensions",
    "settings.theme": "Theme",
    "settings.themeDescription": "Set the interface theme (light/dark/auto)",
    "settings.language": "Language",
    "settings.languageDescription": "Set the interface language",
    "settings.completionSound": "Completion sound",
    "settings.completionSoundDescription": "Play a sound when a task finishes",
    "settings.showBanner": "Show banner",
    "settings.showBannerDescription": "Show the restricted-mode banner for untrusted projects",
    "settings.mcp": "MCP",
    "settings.mcpDescription": "Connect external tools through MCP-compatible extensions",
    "settings.mobile": "Phone access",
    "settings.mobileDescription": "Track progress and send instructions from your phone",
    "settings.mobileLoading": "Loading…",
    "settings.mobileBody": "Phone access keeps the desktop server on localhost and puts a small LAN proxy in front of it: your phone signs in with the password below, and nothing on the network reaches the agent without it.",
    "settings.mobileEnable": "Allow phone access",
    "settings.mobilePassword": "Access password",
    "settings.mobilePasswordPlaceholder": "At least {count} characters",
    "settings.mobilePasswordHint": "Sign in as “pi” with this password on your phone. At least {count} characters.",
    "settings.mobilePasswordFirst": "(set a password first)",
    "settings.mobilePasswordRequired": "Set an access password before enabling phone access.",
    "settings.mobilePasswordTooShort": "The password needs at least {count} characters.",
    "settings.mobileSaved": "Saved",
    "settings.mobileStarting": "Starting the LAN proxy…",
    "settings.mobileScanHint": "Your phone must be on the same Wi-Fi. Scan the code, or open the address above.",
    "settings.mobileCliOnly": "This server was started from the command line. Use `PI_WEB_PASSWORD=… npm run dev:lan` instead — the switch above only applies inside the desktop app.",
    "settings.mobilePublicEnable": "Allow public access",
    "settings.mobilePublicNeedsProxyHint": "(turn on phone access first)",
    "settings.mobilePublicOff": "Public access is off",
    "settings.mobilePublicNeedsProxy": "Waiting for phone access to be turned on",
    "settings.mobilePublicMissingBinary": "cloudflared was not found on this machine",
    "settings.mobilePublicNeedsLogin": "Sign in to Cloudflare once: cloudflared tunnel login",
    "settings.mobilePublicStarting": "Starting the tunnel…",
    "settings.mobilePublicRunning": "Connected",
    "settings.mobilePublicError": "The tunnel stopped with an error",
    "settings.mobilePublicRestarts": "restarts: {count}",
    "settings.mobilePublicLogs": "Recent output",
    "settings.mcpBody": "MCP (Model Context Protocol) is connected through compatible Pi extensions, such as pi-mcp-adapter. The list below contains curated, commonly used servers. Click Use to add one to this project; OAuth services will ask you to authorize on first connection.",
    "settings.mcpConfigFiles": "Common MCP configuration files",
    "settings.mcpProject": "Project",
    "settings.mcpGlobal": "Global",
    "settings.mcpBrowsePlugins": "Browse MCP-capable plugins",
    "settings.requiresProject": "Select a project to manage this resource",
    "settings.mcpSearchPlaceholder": "Search common MCP servers…",
    "settings.mcpName": "{en}",
    "settings.mcpSummary": "{en}",
    "settings.mcpOAuth": "OAuth",
    "settings.mcpUse": "Use",
    "settings.mcpAdding": "Adding…",
    "settings.mcpAddedState": "Added",
    "settings.mcpAlreadyAdded": "This MCP server is already in the project configuration.",
    "settings.mcpAdded": "Added {name} to the project. Reloading the session…",
    "settings.mcpTrustRequired": "Trust this project before adding an MCP server.",
    "settings.mcpNoMatches": "No matching MCP servers",
    "settings.mcpEdit": "Edit JSON",
    "settings.mcpTest": "Test",
    "settings.mcpTesting": "Testing…",
    "settings.mcpTestFailed": "MCP test failed",
    "settings.mcpTestPassedResult": "{name} responded as an MCP endpoint.",
    "settings.mcpTestAuthResult": "{name} is reachable but requires authorization.",
    "settings.mcpTestConfiguredResult": "{name} is configured; its local process will start when an MCP tool is used.",
    "settings.mcpTestUnknownResult": "{name} responded, but the MCP protocol could not be confirmed.",
    "settings.mcpTestFailedResult": "{name} test failed: {detail}",
    "settings.mcpLoading": "Loading…",
    "settings.mcpEditorHint": "Edit the selected MCP configuration. Save validates the JSON and reloads the current session.",
    "settings.mcpCancel": "Cancel",
    "settings.mcpSaving": "Saving…",
    "settings.mcpSave": "Save JSON",
    "settings.mcpSaved": "MCP configuration saved. The session was reloaded.",
    "git.refreshStatus": "Refresh Git status",
    "git.close": "Close Git",
    "git.loadingRepository": "Loading repository…",
    "git.notRepository": "This directory is not a Git repository.",
    "git.branch": "Branch",
    "git.sync": "Sync",
    "git.ahead": "{count} commits to push",
    "git.behind": "{count} commits to pull",
    "git.detachedHead": "Detached HEAD",
    "git.noUpstream": "No upstream",
    "git.operation.merge": "Merge",
    "git.operation.rebase": "Rebase",
    "git.operation.cherry-pick": "Cherry-pick",
    "git.operation.revert": "Revert",
    "git.operationInProgress": "{operation} is in progress.",
    "git.operationConflicts": "({count} conflicts)",
    "git.continue": "Continue",
    "git.abort": "Abort",
    "git.changesSummary": "Changes · +{additions} −{deletions}",
    "git.noUnstagedChanges": "No unstaged changes",
    "git.stagedSummary": "Staged · {count}",
    "git.nothingStaged": "Nothing staged",
    "git.commit": "Commit",
    "git.commitMessage": "Commit message",
    "git.summarizeCommit": "AI summary",
    "git.summarizeCommitTitle": "Generate a commit message only from staged code changes",
    "git.summarizeCommitUnavailable": "Select a session to use its model for a commit message",
    "git.commitStaged": "Commit staged",
    "git.remote": "Remote",
    "git.editRemote": "Edit",
    "git.remoteUrl": "Remote URL",
    "git.cancelEditRemote": "Cancel",
    "git.saveRemote": "Save",
    "git.httpsCredentials": "HTTPS credentials",
    "git.sshCredentials": "SSH key passphrase",
    "git.username": "Username",
    "git.passwordOrToken": "Password or access token",
    "git.keyPassphrase": "Private key passphrase",
    "git.rememberCredential": "Remember encrypted credential on this device",
    "git.savedCredential": "Saved encrypted credential will be used when this field is empty",
    "git.nativeProtocolAnonymous": "The git:// protocol does not support authentication.",
    "git.fetch": "Fetch",
    "git.pullRebase": "Pull (rebase)",
    "git.pullMerge": "Pull (merge)",
    "git.push": "Push",
    "git.rebaseWhenPulling": "Rebase when pulling",
    "git.mergeBranch": "Merge branch",
    "git.selectBranch": "Select a branch",
    "git.merge": "Merge",
    "git.working": "Working…",
    "git.stage": "Stage",
    "git.stageAll": "Stage all",
    "git.unstage": "Unstage",
    "git.unstageAll": "Unstage all",
    "git.discard": "Discard",
    "git.discardFileConfirm": "Discard changes in {file}?",
    "git.discardAll": "Discard all",
    "git.discardAllTitle": "Restore the working tree to its pre-edit state: undo staged and unstaged changes and delete new untracked files (.gitignore'd files are kept)",
    "git.discardAllConfirm": "Restore the working tree to its pre-edit state? This cannot be undone.",
    "git.discardAllTracked": "{count} tracked files: staged and unstaged changes are undone",
    "git.discardAllUntracked": "{count} untracked files: deleted from disk",
    "git.discardAllNoUntracked": "No untracked files to delete",
    "git.discardAllMore": "… and {count} more",
    "git.discardAllConfirmCancel": "Cancel",
    "git.discardAllConfirmAction": "Discard all",
    "git.reset": "Reset",
    "appUpdate.check": "Update",
    "appUpdate.checking": "Checking for updates…",
    "appUpdate.upToDate": "Pi Desktop is up to date. Click to check again",
    "appUpdate.available": "Pi Desktop v{version} is available. Click to download",
    "appUpdate.failed": "Update check failed. Click to retry",
    "appUpdate.releaseNotes": "Pi Desktop v{version} is available. View release notes",
    "about.title": "Version info",
    "about.piVersion": "pi",
    "about.platform": "Platform",
    "about.currentTag": "Current",
    "about.releaseNotes": "Release notes",
    "about.copyAll": "Copy all",
    "about.copied": "Copied",
    "about.historyHint": "{count} releases. Copy all exports the current versions plus the release notes, handy when reporting an issue.",
    "about.empty": "This build has no bundled release notes.",
    "about.emptyRelease": "No changes were recorded for this release.",
    "about.sectionAdded": "Added",
    "about.sectionChanged": "Changed",
    "about.sectionFixed": "Fixed",
    "about.sectionRemoved": "Removed",
    "about.sectionDeprecated": "Deprecated",
    "about.sectionSecurity": "Security",
    "about.sectionOther": "Other",
    "sidebar.hide": "Hide sidebar",
    "sidebar.show": "Show sidebar",
    "theme.light": "Light mode (click for dark)",
    "theme.dark": "Dark mode (click for system)",
    "theme.auto": "System theme (click for light)",
    "history.full": "Full history",
    "history.unsaved": "Full history is available after the session is saved",
    "history.label": "Full history",
    "title.generating": "Generating...",
    "title.updated": "Title updated",
    "title.failed": "Generation failed",
    "title.generate": "Generate title",
    "title.unsaved": "Title generation is available after the session is saved",
    "title.noMessages": "Send a message before naming this session",
    "title.generateSession": "Generate a session title",
    "system.prompt": "System prompt",
    "system.empty": "System prompt is empty (tools are disabled)",
    "system.load": "System prompt has not loaded yet",
    "system.loading": "Loading system prompt…",
    "system.label": "System",
    "session.info": "Session info",
    "session.name": "Name",
    "session.file": "File",
    "session.inMemory": "In-memory",
    "session.id": "ID",
    "session.user": "User",
    "session.assistant": "Assistant",
    "session.toolCalls": "Tool Calls",
    "session.toolResults": "Tool Results",
    "session.total": "Total",
    "session.input": "Input",
    "session.output": "Output",
    "session.cacheRead": "Cache Read",
    "session.cacheWrite": "Cache Write",
    "session.cost": "Cost",
    "session.context": "Context",
    "session.tooltipInput": "in",
    "session.tooltipOutput": "out",
    "session.tooltipCacheRead": "cache read",
    "session.tooltipCacheWrite": "cache write",
    "session.tooltipCost": "cost",
    "session.tooltipContext": "context: {percent} of {tokens} tokens",
    "session.durationSeconds": "{count}s",
    "session.durationMinutes": "{minutes}m {seconds}s",
    "session.durationHours": "{hours}h {minutes}m",
    "session.cacheHitRate": "Avg cache hit rate",
    "session.copy": "Copy {value}",
    "session.copied": "Copied",
    "session.title": "Session info",
    "session.load": "Send a message or run /session to load session info",
    "session.infoSection": "Session Info",
    "session.messages": "Messages",
    "session.tokens": "Tokens",
    "session.totalActive": "Active Time",
    "session.copyFile": "Copy file path",
    "session.copyId": "Copy session ID",
    "workspace.opening": "Opening workspace...",
    "workspace.unable": "Unable to open workspace",
    "workspace.selectSession": "Select a session from the sidebar",
    "workspace.getStarted": "Get Started",
    "workspace.selectProject": "Select a project directory from the sidebar",
    "workspace.addModels": "Add models via the Models button at the bottom",
    "workspace.stepProject": "Select a project directory from the sidebar",
    "workspace.stepModels": "Add models via the Models button at the bottom",
    "files.hidePanel": "Hide file panel",
    "files.showPanel": "Show file panel",
    "files.noneOpen": "No file open",
    "console.title": "Console",
    "console.resize": "Resize terminal height",
    "console.restart": "Restart terminal",
    "console.minimize": "Minimize (keep running)",
    "console.close": "Close terminal and end shell",
    "console.desktopOnly": "The terminal is available in the desktop app. Open this project in Pi Desktop to use it.",
    "console.exited": "The shell has exited. Restart to open a new terminal.",
    "console.backendUnavailable": "The terminal is built into the desktop app; no plugin installation is needed. This desktop executable is missing a command or permission. For development, quit and rerun npm run desktop:dev. For installed apps, install a release containing the terminal fix.",
    "console.open": "Open console",
    "console.clear": "Clear console",
    "console.help": "Show console commands",
    "console.output": "Console output",
    "console.prompt": "Console command",
    "console.placeholder": "Type help, pwd, echo...",
    "console.empty": "Console cleared",
    "console.noProject": "No project selected",
    "console.noSession": "No saved session",
    "console.ready": "Simulated console ready. Type help to see commands.",
    "console.context": "cwd: {cwd}",
    "layout.resizeSidebar": "Resize sidebar",
    "layout.resizeFilePanel": "Resize file panel",
    "layout.resizeHint": "Drag to resize. Double-click or press Enter to reset.",
    "sidebar.new": "New",
    "sidebar.newSessionTitle": "New session in {path}",
    "sidebar.refresh": "Refresh session list",
    "sidebar.selectProject": "Select project…",
    "sidebar.filterProjects": "Filter projects…",
    "sidebar.noMatchingProjects": "No matching projects",
    "sidebar.projectHistory": "Recently added",
    "sidebar.removeProjectHistory": "Remove {path} from recently added",
    "sidebar.customPath": "Custom path…",
    "directoryPicker.selectDirectory": "Select directory",
    "directoryPicker.goToParent": "Go to parent directory",
    "directoryPicker.directoryPath": "Directory path",
    "directoryPicker.goToDirectory": "Go to directory",
    "directoryPicker.go": "Go",
    "directoryPicker.loadingDirectories": "Loading directories…",
    "directoryPicker.noSubdirectories": "No subdirectories",
    "directoryPicker.openBeforeSelecting": "Open this path before selecting it",
    "directoryPicker.selectCurrentDirectory": "Select current directory",
    "directoryPicker.selectThisFolder": "Select this folder",
    "directoryPicker.noDrives": "No drives found",
    "sidebar.checking": "Checking…",
    "sidebar.open": "Open",
    "sidebar.cancel": "Cancel",
    "sidebar.openRepoRoot": "Open repo root",
    "sidebar.openRepoRootTitle": "Open the repository root to manage worktrees.",
    "sidebar.gitRepoRootOnly": "Git repo root only",
    "sidebar.gitRepoRootOnlyTitle": "Worktrees are available in Git repository roots.",
    "sidebar.worktrees": "Worktrees...",
    "sidebar.checkingWorktrees": "Checking worktrees for this directory.",
    "sidebar.filterWorktrees": "Filter worktrees…",
    "sidebar.localBranches": "Local branches",
    "sidebar.noLocalBranches": "No local branches",
    "sidebar.remoteBranches": "Remote branches",
    "sidebar.noRemoteBranches": "No remote branches",
    "sidebar.noMatchingWorktrees": "No matching worktrees",
    "sidebar.switchWorktree": "Switch worktree",
    "sidebar.switchWorktreeTitle": "Switch worktree: {path}",
    "sidebar.forceRemoveCheckout": "Uncommitted changes. Force remove checkout?",
    "sidebar.force": "Force",
    "sidebar.createWorktreeTitle": "Create a worktree checkout for a branch",
    "sidebar.newWorktree": "New worktree…",
    "sidebar.branchName": "branch name",
    "sidebar.creating": "Creating…",
    "sidebar.create": "Create",
    "sidebar.checkout": "Switch",
    "sidebar.checkOut": "Checkout",
    "sidebar.current": "Current",
    "sidebar.newBranch": "Create new branch…",
    "sidebar.createLocalBranch": "Create local branch",
    "sidebar.renameLocalBranch": "Rename local branch",
    "sidebar.renameBranch": "Rename…",
    "sidebar.mergeBranch": "Merge…",
    "sidebar.mergeBranchTitle": "Merge branch",
    "sidebar.mergeBranchDescription": "Merge branch {branch} into:",
    "sidebar.mergeTargetBranch": "Target branch",
    "sidebar.selectBranch": "Select a branch",
    "sidebar.merge": "Merge",
    "sidebar.deleteLocalBranch": "Delete local branch",
    "sidebar.deleteRemoteBranch": "Delete remote branch",
    "sidebar.deleteBranchDescription": "Branch {branch} will be deleted. This cannot be undone.",
    "sidebar.deleteBranchAndWorktreeDescription": "Branch {branch} and its linked worktree will be deleted. This cannot be undone.",
    "sidebar.forceDeleteBranchWorktreeDescription": "The linked worktree has uncommitted changes. Confirm to force remove it and delete branch {branch}.",
    "sidebar.newBranchPrompt": "New branch name",
    "sidebar.renameBranchPrompt": "New branch name",
    "sidebar.deleteBranchConfirm": "Delete branch {branch}? This cannot be undone.",
    "sidebar.deleteBranchAndWorktreeConfirm": "Delete branch {branch} and its linked worktree? This cannot be undone.",
    "sidebar.forceDeleteBranchWorktreeConfirm": "The linked worktree has uncommitted changes. Force remove it and delete branch {branch}?",
    "sidebar.deleteRemoteBranchConfirm": "Delete remote branch {branch}? This cannot be undone from this menu.",
    "sidebar.update": "Update",
    "sidebar.pull": "Pull",
    "sidebar.push": "Push",
    "sidebar.uploadFilesTitle": "Upload files to project root",
    "sidebar.uploadFiles": "Upload files",
    "sidebar.refreshExplorer": "Refresh explorer",
    "sidebar.changedFiles": "{count} changed files",
    "sidebar.loading": "Loading...",
    "sidebar.noSessions": "No sessions found",
    "sidebar.searchSessions": "Search conversations…",
    "sidebar.clearSearch": "Clear conversation search",
    "sidebar.searchingSessions": "Searching conversations…",
    "sidebar.noMatchingSessions": "No matching conversations",
    "sidebar.searchFailed": "Conversation search failed",
    "sidebar.agentRunning": "Agent running…",
    "sidebar.waitingForUser": "Waiting for your answer",
    "sidebar.newActivity": "New activity",
    "sidebar.newSessionActivity": "New session activity",
    "sidebar.deleteSession": "Delete {title}?",
    "sidebar.rename": "Rename",
    "sidebar.delete": "Delete",
    "sidebar.deleteWithShiftClick": "Delete (Shift+click to delete without confirmation)",
    "sidebar.expandForks": "Expand forks",
    "sidebar.collapseForks": "Collapse forks",
    "sidebar.messagesCount": "{count} msgs",
    "sidebar.worktreePath": "Worktree: {path}",
    "sidebar.removeWorktreeTitle": "Remove worktree checkout {path}; the branch is kept",
    "files.modified": "Modified",
    "files.added": "Added",
    "files.deleted": "Deleted",
    "files.renamed": "Renamed",
    "files.untracked": "Untracked",
    "files.conflict": "Conflict",
    "files.newlyUploaded": "Newly uploaded",
    "files.containsChangedFiles": "Contains changed files",
    "files.insertPath": "Insert path into chat",
    "files.mention": "mention",
    "files.reveal": "Show in Finder / file manager",
    "files.revealFailed": "Could not open the file location",
    "files.download": "Download file",
    "files.dismissError": "Dismiss error",
    "files.addUploadedFile": "Add uploaded file to chat",
    "files.addAllUploadedFiles": "Add all uploaded files to chat",
    "files.dismissUploadResults": "Dismiss upload results",
    "files.loading": "Loading files...",
    "files.noFiles": "No files found",
    "files.explorer": "Explorer",
    "files.changedCount": "{count} files",
    "files.changeStats": "{count} changed files, {additions} lines added, {deletions} lines deleted",
    "files.conflictSummary": "{count} file{countSuffix} already exist: {files}",
    "files.cannotReplace": "Cannot replace: {files}",
    "files.replace": "Replace",
    "files.skipExisting": "Skip existing",
    "files.cancel": "Cancel",
    "files.checking": "Checking files",
    "files.uploading": "Uploading, {progress}%",
    "chat.loadingSession": "Loading session...",
    "chat.runningTool": "Running tool...",
    "chat.generatingToolInput": "Generating parameters...",
    "chat.runningNamedTool": "Running {name}...",
    "chat.runningTools": "Running {names}...",
    "chat.runningToolsMore": "Running {names} (+{count})...",
    "chat.waitingModel": "Waiting for model...",
    "chat.runningCommand": "Running command...",
    "chat.thinking": "Thinking...",
    "chat.processDetails": "Process details",
    "chat.message": "message",
    "chat.messages": "messages",
    "chat.toolCall": "tool call",
    "chat.toolCalls": "tool calls",
    "chat.collapseProcess": "Collapse process details",
    "chat.expandProcess": "Expand process details",
    "chat.filesWritten": "Files changed",
    "chat.openWrittenFile": "Open {name}",
    "chat.loadEarlier": "Scroll up to load earlier messages ({count} hidden)",
    "chat.extensionRequest": "extension request",
    "chat.cancel": "Cancel",
    "chat.confirm": "Confirm",
    "chat.submit": "Submit",
    "chat.extensionInput": "Extension terminal input",
    "chat.extensionPanel": "Extension panel",
    "chat.extensionWidgets": "Extension widgets",
    "chat.extensionWidgetAbove": "Above editor widget",
    "chat.extensionWidgetBelow": "Below editor widget",
    "chat.extensionWidgetLine": "{count} line",
    "chat.extensionWidgetLines": "{count} lines",
    "chat.close": "Close",
    "chat.queued": "Queued · {count}",
    "chat.recall": "Recall to input",
    "chat.recallTitle": "Remove all queued messages and put them back into the input box for editing",
    "chat.retrying": "Retrying ({attempt}/{max})…",
    "chat.promptFailed": "Request failed: {error}",
    "chat.retryPrompt": "Retry",
    "chat.loadingCommands": "Loading commands...",
    "chat.slashCommands": "Slash commands · {label}",
    "chat.tabEnter": "Tab / Enter",
    "chat.noCommands": "No extension, prompt, or skill commands found",
    "chat.match": "1 match",
    "chat.matches": "{count} matches",
    "chat.command": "1 command",
    "chat.commands": "{count} commands",
    "chat.loadingFiles": "Loading files...",
    "chat.files": "Files · {label}{hint}",
    "chat.searching": "Searching…",
    "chat.noMatchingFiles": "No matching files",
    "chat.searchingAll": " · searching all files…",
    "chat.indexTruncated": " · index truncated",
    "chat.steerPlaceholder": "Steer now / queue...",
    "chat.agentPlaceholder": "Agent is running…",
    "chat.messagePlaceholder": "Type / for commands, @ for files",
    "chat.chatModePlaceholder": "Just chat…",
    "chat.switchToChatMode": "Switch to plain conversation mode (no tools)",
    "chat.switchToWorkMode": "Switch back to work mode",
    "chat.toolPresetChatMode": "Plain conversation mode uses no tools",
    "chat.commandArgsPlaceholder": "Add details…",
    "chat.steer": "Steer",
    "chat.followUp": "Queue",
    "chat.changeModel": "Change model",
    "chat.switchingModel": "Switching model",
    "chat.selectModel": "Select model",
    "chat.noModels": "No models",
    "chat.noAvailableModels": "No available models",
    "chat.modelError": "Model error",
    "chat.modelScopeWarning": "Model scope warning",
    "chat.modelScopeWarnings": "Model scope warnings",
    "chat.send": "Send",
    "chat.attachImage": "Attach image",
    "chat.previewImage": "Preview image",
    "chat.filterModels": "Filter models…",
    "chat.noMatchingModels": "No matching models",
    "chat.moreControls": "More controls",
    "chat.collapseControls": "Collapse controls",
    "chat.shell": "Shell",
    "chat.outputLocal": "output stays local",
    "chat.outputModel": "output sent to model",
    "chat.noTools": "No tools",
    "chat.readOnlyTools": "{count} read-only built-in tools",
    "chat.builtInTools": "{count} built-in tools",
    "chat.allBuiltInTools": "All built-in tools",
    "chat.changeReasoning": "Change reasoning level: {level}",
    "chat.changeReasoningLabel": "Change reasoning level",
    "chat.changeToolPreset": "Change tool preset",
    "chat.stopCompaction": "Stop compaction",
    "chat.compactContext": "Compact context",
    "chat.compacting": "Compacting…",
    "chat.compact": "Compact",
    "chat.stopAgent": "Stop agent",
    "chat.stop": "Stop",
    "chat.disableSound": "Disable completion sound",
    "chat.enableSound": "Enable completion sound",
    "chat.thinkingUseDefault": "Use pi default",
    "chat.thinkingOff": "Reasoning off",
    "chat.thinkingMinimal": "Minimal reasoning",
    "chat.thinkingLow": "Low reasoning",
    "chat.thinkingMedium": "Medium reasoning",
    "chat.thinkingHigh": "High reasoning",
    "chat.thinkingXhigh": "Extra-high reasoning",
    "chat.thinkingMax": "Max reasoning",
    "chat.builtIn": "Built-in",
    "chat.extensions": "Extensions",
    "chat.prompts": "Prompts",
    "chat.skills": "Skills",
    "chat.chooseSkill": "Choose skill",
    "chat.skillsAvailable": "Skills · {count} available",
    "chat.searchSkills": "Search skills",
    "chat.noSkillsAvailable": "No skills are available for this folder",
    "chat.noMatchingSkills": "No matching skills",
    "chat.skillsLoadFailed": "Skills could not be loaded",
    "chat.officialSkills": "Official / built-in",
    "chat.projectSkills": "Project",
    "chat.globalSkills": "Global",
    "chat.otherSkills": "Other sources",
    "chat.dormant": "dormant",
    "chat.commandCompact": "Compress context, optionally with instructions",
    "chat.commandReload": "Reload extensions, skills, prompts, and tools",
    "chat.commandName": "Set the session display name",
    "chat.commandSession": "Show session message, token, and cost stats",
    "chat.commandCopy": "Copy the last assistant message",
    "chat.compacted": "Compacted",
    "chat.tokensSaved": "{saved} saved",
    "i18n.close": "Close",
    "i18n.copy": "Copy",
    "i18n.copied": "Copied",
    "i18n.cancel": "Cancel",
    "i18n.save": "Save",
    "i18n.saving": "Saving…",
    "i18n.saved": "Saved",
    "i18n.remove": "Remove",
    "i18n.removing": "Removing…",
    "i18n.delete": "Delete",
    "i18n.loading": "Loading...",
    "i18n.search": "Search",
    "i18n.refresh": "Refresh",
    "i18n.update": "Update",
    "i18n.updating": "Updating...",
    "i18n.check": "Check",
    "i18n.checking": "Checking...",
    "i18n.install": "Install",
    "i18n.installing": "Installing...",
    "i18n.noResults": "No results",
    "i18n.searchProviders": "Search providers…",
    "i18n.noProviders": "No providers match",
    "i18n.newModel": "new model",
    "i18n.addProvider": "Add provider",
    "i18n.selectProviderModel": "Select a provider or model",
    "i18n.visibleInPrompt": "Visible in model prompt — click to disable",
    "i18n.hiddenFromPrompt": "Hidden from model prompt — click to enable",
    "i18n.dormant": "Dormant",
    "i18n.hiddenButInvocable": "Hidden from model, still manually invocable",
    "i18n.skillSearchPlaceholder": "e.g. react, testing, deploy",
    "i18n.searching": "Searching…",
    "i18n.noSkills": "No skills found",
    "i18n.checkUpdates": "Check updates",
    "i18n.updates": "updates",
    "i18n.noResolvedResources": "No resolved resources",
    "i18n.enablePackage": "Enable package",
    "i18n.disablePackage": "Disable package",
    "i18n.filtered": "filtered",
    "i18n.openSessionToReload": "Open a session to reload",
    "i18n.notFound": "Not found",
    "i18n.noPlugins": "No plugins configured",
    "trust.resourcesNotLoaded": "Restricted mode",
    "trust.trustProject": "Trust project",
    "trust.trusting": "Trusting...",
    "trust.cancel": "Cancel",
    "trust.dialogTitle": "Trust this project?",
    "trust.dialogBody": "Project resources can run local code. Trust only projects whose contents you know.",
    "trust.skillsNotLoaded": "Project skills are not loaded because this project is not trusted.",
    "trust.pluginsNotLoaded": "Project plugins are not loaded because this project is not trusted.",
    "trust.projectScopeUnavailable": "Project installs are unavailable while project resources are not loaded.",
    "i18n.creatingSession": "Creating new session…",
    "i18n.creating": "Creating…",
    "i18n.subscription": "Subscription",
    "i18n.subscriptions": "Subscriptions",
    "i18n.customEndpoint": "Custom endpoint format",
    "i18n.noChanges": "No changes",
    "i18n.branches": "Conversation branches",
    "i18n.noActiveSession": "No active session",
    "i18n.noBranches": "This session has no branches",
    "i18n.source": "Source",
    "i18n.preview": "Preview",
    "i18n.diff": "Diff",
    "i18n.downloadFile": "Download file",
    "i18n.liveSync": "Live sync active",
    "i18n.notWatching": "Not watching",
    "i18n.fileViewMode": "File view mode",
    "i18n.compareHead": "Compare working tree with HEAD",
    "i18n.mentionSelectedLines": "Mention selected lines",
    "i18n.disableWrap": "Disable word wrap",
    "i18n.enableWrap": "Enable word wrap",
    "i18n.htmlPreview": "HTML preview",
    "i18n.previewFile": "Preview {file}",
    "i18n.invalidMermaid": "Invalid Mermaid diagram",
    "i18n.renderingMermaid": "Rendering Mermaid diagram",
    "i18n.previewAfterStreaming": "Preview available after streaming",
    "i18n.showMermaidSource": "Show Mermaid source",
    "i18n.previewMermaid": "Preview Mermaid diagram",
    "i18n.openMermaidViewer": "Open diagram viewer",
    "i18n.mermaidViewer": "Mermaid diagram viewer",
    "i18n.mermaidDiagram": "Mermaid diagram",
    "i18n.zoomOut": "Zoom out",
    "i18n.zoomIn": "Zoom in",
    "i18n.fitToWidth": "Fit to width",
    "models.discoveryFetching": "Importing models…",
    "models.discoveryFetch": "Import models…",
    "models.discoveryFilterPlaceholder": "Filter {count} models…",
    "models.discoveryFilter": "Filter upstream models",
    "models.discoverySelectShown": "Select shown",
    "models.discoveryNoMatches": "No matching models.",
    "models.discoveryAdded": "added",
    "models.discoveryShowing": "Showing first {shown} of {total}",
    "models.discoveryFetched": "{count} models fetched",
    "models.discoveryAddSelected": "Add selected",
    "models.discoveryAddSelectedCount": "Add selected ({count})",
    "models.catalogSource": "source: models.dev ↗",
    "models.catalogFilling": "Filling details…",
    "models.catalogFill": "Fill model details",
    "models.catalogFilled": "{count} fields filled",
    "models.catalogNoEmptyFields": "No empty fields to fill",
    "models.catalogNoExactMatch": "No exact models.dev match",
    "models.catalogPriceProvider": "Price matched Provider {provider}",
    "models.catalogPriceBaseUrl": "Price matched Base URL to {provider}",
    "models.catalogPriceConsensus": "Price agreed by {support}/{total} records",
    "models.catalogPriceUnreliable": "No reliable price found",
    "models.catalogUndo": "Undo",
    "models.visibleTitle": "Model picker",
    "models.visibleSelectedCount": "{selected}/{total} selected",
    "models.visibleUnscoped": "No whitelist set: every available model shows up in the chat model picker.",
    "models.visibleScoped": "Whitelist active: only the checked models show up in the chat model picker.",
    "models.visibleShowAll": "Check all",
    "models.visibleHideAll": "Uncheck all",
    "models.visibleSearch": "Filter models…",
    "models.visibleEmpty": "No known models for this provider yet.",
    "models.visibleRefreshing": "Loading models…",
    "models.visibleSaving": "Saving…",
    "models.visibleSaved": "Saved",
    "models.visibleKeepOne": "At least one model must stay visible.",
    "models.visibleRetry": "Retry",
    "mobile.pairTitle": "Phone access",
    "mobile.pairLoading": "Preparing…",
    "mobile.pairScanHint": "Same Wi-Fi as this computer. Scan to open the remote view on your phone.",
    "mobile.pairAuthHint": "Sign in as “pi” with your access password the first time.",
    "mobile.pairCopy": "Copy link",
    "mobile.pairCopied": "Copied",
    "mobile.pairPublicHint": "Works on cellular too — same username and password. On the same Wi-Fi it switches to the local network by itself.",
    "mobile.pairPublicOff": "Public access is off: enable it in Settings → Phone access → Public access.",
    "mobile.lanSwitchTitle": "Switching to your local network…",
    "mobile.lanSwitchFallbackHint": "Your phone shares this computer's public address but not its local network. Continue over the internet:",
    "mobile.lanSwitchFallbackAction": "Continue over the internet",
    "mobile.pairLanRequired": "This server only listens on localhost, so your phone cannot reach it. Start it in LAN mode:",
    "mobile.pairLanDisabled": "Phone access is off. Turn it on in Settings → Phone access, then scan again.",
    "mobile.pairStarting": "Starting the LAN proxy… try again in a moment.",
    "mobile.pairNoAddress": "No LAN address found. Check this computer's network connection, then reopen this dialog.",
    "mobile.demoNotice": "Demo data · the real session wires up in step 2",
    "mobile.statusRunning": "Running",
    "mobile.statusIdle": "Idle",
    "mobile.stop": "Stop",
    "mobile.send": "Send",
    "mobile.placeholder": "Send an instruction…",
    "mobile.queued": "{count} queued",
    "mobile.emptySession": "Nothing here yet. Send an instruction below.",
    "mobile.placeholderQueued": "Agent is running. Send to queue another instruction…",
    "mobile.needProject": "This page needs a project. Re-open it from the desktop QR code.",
    "mobile.newTask": "New task",
    "mobile.newTaskHint": "The first instruction starts a new session in this project.",
    "mobile.jumpToLatest": "Jump to latest",
    "mobile.loading": "Loading…",
    "mobile.loadEarlier": "Load earlier messages ({count} more)",
    "mobile.historyCapped": "Older messages are only on the desktop.",
    "mobile.promptCustomHint": "The extension opened a custom interface. Answer it on the desktop, or cancel to unblock this run.",
    "mobile.you": "You",
    "mobile.assistant": "AI",
    "models.capabilities": "Capabilities",
    "models.reasoning": "Reasoning / thinking",
    "models.imageInput": "Image input",
    "models.capabilityText": "Text",
    "models.capabilityTextImage": "Text + image",
    "models.modelSpecs": "Model specifications",
    "models.contextWindow": "Context window (tokens)",
    "models.maxOutputTokens": "Max output tokens",
    "models.costPerMillion": "Cost per million tokens",
    "models.costInput": "Input",
    "models.costOutput": "Output",
    "models.costCacheRead": "Cache read",
    "models.costCacheWrite": "Cache write",
    "models.notProvided": "Not provided",
    "models.editCosts": "Edit prices",
    "models.finishEditingCosts": "Done",
    "models.costAllRequired": "Enter a valid non-negative price; blank fields are saved as 0.",
    "models.advancedSettings": "Advanced settings",
    "models.providerDefaults": "Using provider defaults",
    "models.headersSummary": "Headers: {count}",
    "models.compatSummary": "Compatibility: {count}",
    "models.thinkingSummary": "Thinking levels: {count}",
    "models.apiOverride": "API override",
    "models.headers": "Headers",
    "models.headersHelp": "Added to this model's requests and overrides provider headers.",
    "models.compatibility": "Compatibility",
    "models.deepSeekThinkingCompat": "DeepSeek thinking compatibility",
    "models.developerRole": "Use the developer role for the system prompt",
    "models.thinkingLevelMap": "Thinking level map",
    "models.clearAll": "Clear all",
    "i18n.testingModel": "Testing model connection...",
    "i18n.connected": "Connected",
    "i18n.failed": "Failed",
    "i18n.test": "Test",
    "i18n.testConnection": "Test model connection",
    "i18n.provider": "Provider",
    "i18n.model": "Model",
    "i18n.providerName": "Provider name",
    "i18n.rename": "Rename",
    "i18n.default": "Default",
    "i18n.disabled": "Disabled",
    "i18n.noResources": "No resources",
    "i18n.unknown": "Unknown",
    "i18n.installedVersion": "installed {version}",
    "i18n.configuredVersion": "configured {version}",
    "i18n.extensions": "Extensions",
    "i18n.prompts": "Prompts",
    "i18n.themes": "Themes",
    "i18n.resourceCount": "{count} {label}",
    "i18n.extensionShort": "ext",
    "i18n.skillShort": "skills",
    "i18n.promptShort": "prompts",
    "i18n.themeShort": "themes",
    "i18n.custom": "Custom",
    "i18n.clearAll": "clear all",
    "i18n.connectedSuccessfully": "Connected successfully.",
    "i18n.notConnected": "not connected",
    "i18n.configured": "configured",
    "i18n.notConfigured": "not configured",
    "i18n.openingBrowser": "Opening browser…",
    "i18n.submit": "Submit",
    "i18n.login": "Login",
    "i18n.relogin": "Re-login",
    "i18n.disconnect": "Disconnect",
    "i18n.apiKeyStoredHint": "Your API key is stored. Edit the field below to replace it, or disconnect to remove it.",
    "i18n.apiKeyMissingHint": "Enter your {provider} API key to enable {count} models.",
    "i18n.apiKeyNewPlaceholder": "Enter a new API key…",
    "i18n.networkError": "Network error",
    "i18n.continuing": "Continuing…",
    "i18n.skills": "Skills",
    "i18n.plugins": "Plugins",
    "i18n.addSkill": "Add skill",
    "i18n.addPlugin": "Add plugin",
    "i18n.discoverPlugins": "Discover plugins",
    "i18n.pluginCatalogIntro": "Browse the built-in Pi package catalog, sorted by usage on the npm registry (monthly downloads) from most to least. Each item shows a reviewed Chinese summary when available, plus the author's original description; online searches can add newer packages.",
    "i18n.searchPlugins": "Search plugins by name or keyword…",
    "i18n.pluginCatalogLoading": "Loading plugin catalog…",
    "i18n.noPluginMatches": "No matching plugins found",
    "i18n.noChineseDescription": "Chinese summary not available; see the original description below.",
    "i18n.noLocalizedDescription": "Translation unavailable; see the original description below.",
    "i18n.machineTranslated": "machine translated",
    "i18n.translatingDescription": "Translating the description…",
    "i18n.loadMorePlugins": "Load more",
    "i18n.pluginMonthlyInstalls": "{count}/mo",
    "i18n.pluginWeeklyInstalls": "{count}/wk",
    "i18n.pluginUsageTitle": "Usage on the npm registry (last 30 days): {count}",
    "i18n.pluginUsageWeeklyTitle": "Usage on the npm registry (last 7 days): {count}",
    "i18n.manualPluginInstall": "Advanced: install from a package source",
    "i18n.installTo": "Install to",
    "i18n.retry": "Retry",
    "i18n.selectSkill": "Select a skill",
    "i18n.selectPackage": "Select a package",
    "i18n.updateAvailable": "Update available",
    "i18n.upToDate": "Up to date",
    "i18n.automaticChecksUnavailable": "Automatic checks unavailable",
    "i18n.checkFailed": "Check failed",
    "i18n.installed": "Installed",
    "i18n.status": "Status",
    "i18n.version": "Version",
    "i18n.package": "Package",
    "i18n.resources": "Resources",
    "i18n.installedPath": "Installed path",
    "i18n.cwd": "CWD",
    "i18n.resolvedResources": "Resolved Resources",
    "i18n.reloadSession": "Reload session",
    "i18n.reloading": "Reloading...",
    "i18n.packageRemoved": "Package removed.",
    "i18n.packageInstalled": "Package installed.",
    "i18n.packageUpdated": "Package updated.",
    "i18n.packageDisabled": "Package disabled.",
    "i18n.packageEnabled": "Package enabled.",
    "i18n.sessionReloaded": "Session reloaded.",
    "i18n.thinking": "Thinking",
    "i18n.largeMessageReveal": "Message content is very large ({size}). Click to view as plain text — markdown rendering is disabled to keep the page responsive.",
    "i18n.loadingThinking": "Loading thinking...",
    "i18n.copyMessage": "Copy message",
    "i18n.editFromHere": "Edit from here",
    "i18n.editFromHereTitle": "Edit from here — branches within this session",
    "i18n.newSession": "New session",
    "i18n.newSessionTitle": "New session — creates an independent copy from here",
    "i18n.estimatedTokens": "Estimated token count while streaming",
    "i18n.noOutput": "(no output)",
    "i18n.noSummary": "(no summary)",
    "i18n.conversationCompacted": "Conversation compacted",
    "i18n.compactionDescription": "The conversation history before this point was compacted into the following summary:",
    "i18n.fileContext": "File context: {details}",
    "i18n.readFiles": "Read files",
    "i18n.modifiedFiles": "Modified files",
    "i18n.hiddenExtensionMessage": "hidden extension message",
    "i18n.noMessage": "(no message)",
    "i18n.showExtensionMessage": "Show extension message",
    "i18n.collapse": "Collapse",
    "i18n.expand": "Expand",
    "i18n.hideDetails": "Hide details",
    "i18n.showDetails": "Show details",
    "i18n.thinkingUnavailable": "Thinking content unavailable",
    "i18n.before": "Before",
    "i18n.after": "After",
    "i18n.sessionComplete": "Session complete",
    "i18n.taskFinished": "Task finished.",
    "i18n.attentionNeeded": "Pi needs your attention",
    "i18n.extensionInputNeeded": "An extension is waiting for your input.",
  },
};
