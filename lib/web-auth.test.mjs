...
test("rate limits repeated failed authentication attempts within one window", async () => {
  const { recordFailedWebAuthAttempt } = await loadSubject();
  const attempts = [];

  for (let index = 0; index < 20; index += 1) {
    assert.equal(recordFailedWebAuthAttempt(attempts, 10_000 + index).rateLimited, false);
  }
  const blocked = recordFailedWebAuthAttempt(attempts, 10_020);
  assert.equal(blocked.rateLimited, true);
  assert.equal(blocked.retryAfterSeconds, 60);

  assert.equal(recordFailedWebAuthAttempt(attempts, 70_020).rateLimited, false);
  assert.deepEqual(attempts, [70_020]);
});
