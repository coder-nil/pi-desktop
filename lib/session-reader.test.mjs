...
test("hides transcript system messages from the chat context", () => {
  // Pi >= 0.86 persists the prompt, the tool loadout and section patches as
  // `role: "system"` entries. They are provider input, so the UI must skip them
  // while still letting the SDK decide which entries are on the active path.
  const entries = [
    {
      type: "message",
      id: "sys1",
      ...
      message: { role: "system", content: "You are pi.", sections: { tools: "read, bash" } },
    },
    userEntry("u1", "sys1", "hello"),
...
test("keeps entry IDs aligned when a system message is dropped", () => {
  // entryIds is parallel to messages; fork and navigate_tree target these ids, so a
  // hidden entry must not shift them.
  const entries = [
    userEntry("u1", null, "first"),
    {
      ...
      message: { role: "system", content: "", sections: { tools: "read" } },
    },
    assistantEntry("a1", "sys2", "second"),
  ];
...
test("reads only a bounded session header, including headers larger than 4 KiB", () => {
  const dir = mkdtempSync(join(tmpdir(), "pi-desktop-header-"));
...
test("returns null for malformed or unbounded session headers", () => {
  const dir = mkdtempSync(join(tmpdir(), "pi-desktop-header-invalid-"));
...
test("keeps forward and reverse session path caches in sync", async () => {
  ...
  const filePath = join(tmpdir(), "pi-desktop-cache-test", "..", "cache-test", "session.jsonl");
...
test("evicts a cached path when its session file no longer exists", async (t) => {
  const sessionId = "stale-cache-test-session";
  const missingPath = join(tmpdir(), "pi-desktop-missing-session", "session.jsonl");
  ...
  assert.equal(globalThis.__piSessionPathCache?.has(sessionId), false);
  assert.equal(globalThis.__piPathToSessionIdCache?.has(sessionPathKey(missingPath)), false);
});
