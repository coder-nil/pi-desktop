...
test("waits for the source reply before sending the title prompt", async () => {
  ...
  // Pi >= 0.87 prepends the transcript's leading system message, which is where
  // providers now read the prompt from.
  assert.deepEqual(providerRoles, ["system", "user", "assistant", "user"]);
});
...
test("generates a title when compaction removed all literal user messages", async () => {
  ...
  // Index 0 is the leading system message (Pi >= 0.87); the synthesized user
  // message built from the compaction summary follows it.
  assert.deepEqual(providerMessages.map((message) => message.role), ["system", "user", "assistant", "user"]);
  assert.equal(providerMessages[0].content, "system");
  assert.match(providerMessages[1].content[0].text, /fix title generation after compaction/);
});
...
test("removes incomplete tool calls before invoking the title provider", async () => {
  ...
  assert.deepEqual(providerMessages.map((message) => message.role), ["system", "user"]);
  assert.match(providerMessages[1].content, /Create a concise title/);
});
