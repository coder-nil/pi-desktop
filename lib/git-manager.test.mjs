...
test("reports local commits as ahead and upstream commits as behind", async (t) => {
  const cwd = await mkdtemp(path.join(os.tmpdir(), "pi-desktop-git-ahead-behind-"));
  t.after(() => rm(cwd, { recursive: true, force: true }));
  ...
  await writeFile(path.join(cwd, "local.txt"), "local 1\n");
  await git(cwd, "add", "local.txt");
  await git(cwd, "commit", "-m", "local 1");
  await writeFile(path.join(cwd, "local.txt"), "local 2\n");
  await git(cwd, "commit", "-am", "local 2");

  const { getGitSummary } = await loadSubject();
