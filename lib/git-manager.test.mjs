...
test("discards only unstaged tracked changes", async (t) => {
  // realpath avoids the macOS /var -> /private/var symlink mismatch that made
  // getGitStatus filter every file out and turned this test into a no-op.
  const cwd = await realpath(await mkdtemp(path.join(os.tmpdir(), "pi-desktop-git-discard-")));
  t.after(() => rm(cwd, { recursive: true, force: true }));
  await git(cwd, "init", "--initial-branch=main");
  await git(cwd, "config", "user.email", "test@example.com");
  await git(cwd, "config", "user.name", "Test User");
  await writeFile(path.join(cwd, "only-unstaged.txt"), "base\n");
  await writeFile(path.join(cwd, "partially-staged.txt"), "base\n");
  await git(cwd, "add", ".");
  await git(cwd, "commit", "-m", "initial");

  await writeFile(path.join(cwd, "only-unstaged.txt"), "edited\n");
  await writeFile(path.join(cwd, "partially-staged.txt"), "staged\n");
  await git(cwd, "add", "partially-staged.txt");
  await writeFile(path.join(cwd, "partially-staged.txt"), "staged+unstaged\n");
  ...
  // Unstaged edit is gone, staged half survives, untracked file is kept.
  assert.equal(await readFile(path.join(cwd, "only-unstaged.txt"), "utf8"), "base\n");
  assert.equal(await readFile(path.join(cwd, "partially-staged.txt"), "utf8"), "staged\n");
  assert.equal(await readFile(path.join(cwd, "untracked.txt"), "utf8"), "new\n");
  assert.equal((await git(cwd, "status", "--porcelain")).stdout, "M  partially-staged.txt\n?? untracked.txt\n");
  assert.deepEqual(summary.changes.files.map((file) => file.filePath.split(path.sep).pop()).sort(), ["partially-staged.txt", "untracked.txt"]);
});

test("keeps staged-only changes when discarding unstaged work", async (t) => {
  const cwd = await realpath(await mkdtemp(path.join(os.tmpdir(), "pi-desktop-git-discard-staged-")));
  t.after(() => rm(cwd, { recursive: true, force: true }));
  await git(cwd, "init", "--initial-branch=main");
  await git(cwd, "config", "user.email", "test@example.com");
  await git(cwd, "config", "user.name", "Test User");
  ...
  await git(cwd, "commit", "-m", "initial");
  await writeFile(path.join(cwd, "a.txt"), "staged-only\n");
  await git(cwd, "add", "a.txt");

  const { runGitAction } = await loadSubject();
  const summary = await runGitAction(cwd, "discard_all");

  assert.equal((await git(cwd, "status", "--porcelain")).stdout, "M  a.txt\n");
  assert.deepEqual(summary.changes.files.map((file) => file.indexStatus), ["M"]);
});
