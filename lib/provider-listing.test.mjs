...
test("preserves base URLs for frontend favicon lookup", () => {
  const providers = [provider({ id: "apisets", baseUrl: "https://coding.apisets.com/v1" })];

  assert.equal(buildApiKeyProviderList(providers)[0].baseUrl, "https://coding.apisets.com/v1");
  assert.equal(buildOAuthProviderList(providers)[0].baseUrl, "https://coding.apisets.com/v1");
});
