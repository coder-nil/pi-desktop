...
test("maps bare tool names to a preset (set_chat_mode restores the work tool set)", () => {
  assert.equal(getPresetFromActiveNames([]), "none");
  assert.equal(getPresetFromActiveNames(PRESET_DEFAULT), "default");
  assert.equal(getPresetFromActiveNames(["read", "powershell", "edit", "write"]), "default");
  assert.equal(getPresetFromActiveNames([...PRESET_FULL, "web_search"]), "full");
  assert.equal(getPresetFromActiveNames(["web_search"]), "none");
});

test("treats PowerShell as the Windows equivalent of the bash slot", () => {
  assert.equal(
