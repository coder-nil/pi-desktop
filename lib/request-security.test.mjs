...
// 桌面壳会给子进程注入 PI_WEB_HOSTNAME=127.0.0.1，而这里的函数拿它当「服务绑在哪个
// 地址」的默认值。测试必须自己决定有没有配置，不能跟着运行环境变——否则在 pi-desktop
// 自己的 shell 里跑 npm test 就会伪失败（CI 里却通过）。
...
test("allows IPv6 and an explicitly configured hostname", async () => {
  ...
  const configured = new Request("http://localhost:30141/api/test", {
    method: "POST",
    headers: {
      host: "pi-desktop.internal:30141",
      origin: "http://pi-desktop.internal:30141",
      "sec-fetch-site": "same-origin",
    },
  });
  assert.equal(isApiRequestAllowed(ipv6), true);
  assert.equal(isApiRequestAllowed(configured, ["pi-desktop.internal"]), true);
});

test("allows passwordless access only for a loopback bind and Host header", async () => {
  const { isUnauthenticatedLoopbackRequest } = await loadSubject();
  for (const host of [
    "localhost:30141",
    "ui.localhost:30141",
    "127.0.0.1:30141",
    "127.12.34.56:30141",
    "[::1]:30141",
    "[0:0:0:0:0:0:0:1]:30141",
    "[::ffff:127.0.0.1]:30141",
  ]) {
    assert.equal(isUnauthenticatedLoopbackRequest(new Request("http://localhost", {
      headers: { host },
    }), "127.0.0.1"), true, host);
  }

  for (const host of ["0.0.0.0:30141", "192.168.1.9:30141", "pi-desktop.internal:30141"]) {
    assert.equal(isUnauthenticatedLoopbackRequest(new Request("http://localhost", {
      headers: { host },
    }), "127.0.0.1"), false, host);
  }

  const forgedLoopbackHost = new Request("http://localhost", {
    headers: { host: "localhost:30141" },
  });
  assert.equal(isUnauthenticatedLoopbackRequest(forgedLoopbackHost, "0.0.0.0"), false);
  assert.equal(isUnauthenticatedLoopbackRequest(forgedLoopbackHost, undefined), false);
});
...
test("allows only the desktop shell origin configured by the local API server", async () => {
  const originalOrigin = process.env.PI_WEB_DESKTOP_API_ORIGIN;
  process.env.PI_WEB_DESKTOP_API_ORIGIN = "tauri://localhost";
  try {
    const { isApiRequestAllowed } = await loadSubject();
    const desktopRequest = new Request("http://127.0.0.1:30141/api/test", {
      method: "POST",
      headers: {
    ...
    const attackerRequest = new Request("http://127.0.0.1:30141/api/test", {
      method: "POST",
      headers: {
        host: "127.0.0.1:30141",
        origin: "tauri://attacker",
        "sec-fetch-site": "cross-site",
      },
    });
    assert.equal(isApiRequestAllowed(desktopRequest), true);
    assert.equal(isApiRequestAllowed(attackerRequest), false);
  } finally {
    if (originalOrigin === undefined) delete process.env.PI_WEB_DESKTOP_API_ORIGIN;
    else process.env.PI_WEB_DESKTOP_API_ORIGIN = originalOrigin;
  }
});
...
test("rejects missing, malformed, and unconfigured Host headers", async () => {
  ...
  assert.equal(isApiRequestAllowed(new Request("http://localhost:30141/api/test", {
    headers: { host: "pi-desktop.internal:30141" },
  })), false);
});
