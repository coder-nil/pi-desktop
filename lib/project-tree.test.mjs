...
test("does not label a branch with a transcript system message", () => {
  // Pi >= 0.86 records the prompt and tool loadout as `role: "system"` entries, so a
  // branch can start with one. Its preview must come from the first real turn; without
  // the filter it would read "message" (empty content falls back to the role name).
  const system = node(msg("sys1", "system", ""), [node(msg("u2", "user", "真正的问题"), [node(info("leaf1"))])]);
  const sibling = node(msg("u2b", "user", "另一个分支"), [node(info("leaf2"))]);
  const projected = projectTreeForResponse([node(msg("u1", "user", "第一问"), [system, sibling])]);
  const leaf = findProjectedNode(projected, "leaf1");

  assert.deepEqual(leaf.compressedEntryIds, ["sys1", "u2"]);
  assert.deepEqual(leaf.branchPreview, { role: "user", text: "真正的问题" });
});
