...
async function createProjectFixture(t) {
  const root = await mkdtemp(join(tmpdir(), "pi-desktop-project-trust-"));
...
test("all project resource loaders and reloads enforce project trust", async () => {
  ...
  // reload 点会随功能增长（现在就有 prompt 前、手动 reload、切会话三处），所以不数固定个数，
  // 而是断言「每个 `inner.reload` 之前都同步过一次项目信任」——那才是这条用例要守的约束。
  const syncBeforeReload = Array.from(
    rpcSource.matchAll(/this\.syncProjectTrust\(\);\s*await this\.inner\.reload/g),
  ).length;
  const reloadCalls = Array.from(rpcSource.matchAll(/await this\.inner\.reload\(/g)).length;
  assert.ok(syncBeforeReload >= 2);
  assert.equal(syncBeforeReload, reloadCalls);
