...
const { readStoredApiKey, removeStoredCredentialIfType, storeProviderCredential } = await createJiti(import.meta.url)
  .import("./provider-credential-store.ts");

async function withAuthFile(data, run) {
  const dir = await mkdtemp(join(tmpdir(), "pi-desktop-provider-auth-"));
...
test("saving and removing an API key updates the SDK authentication status", async () => {
  await withAuthFile({}, async (authPath) => {
    const providerId = "pi-desktop-auth-status-test";
...
test("reads a stored API key for masking without touching other providers", async (t) => {
  const directory = await mkdtemp(join(tmpdir(), "pi-desktop-credential-read-"));
  t.after(() => rm(directory, { recursive: true, force: true }));
  const authPath = join(directory, "auth.json");
  await storeProviderCredential("deepseek", { type: "api_key", key: "sk-secret-value-4f2a" }, authPath);
  await storeProviderCredential("anthropic", { type: "oauth", access: "token", refresh: "r", expires: Date.now() + 60_000 }, authPath);

  assert.equal(readStoredApiKey("deepseek", authPath), "sk-secret-value-4f2a");
  // OAuth 凭据没有可掩码的 API Key。
  assert.equal(readStoredApiKey("anthropic", authPath), null);
  assert.equal(readStoredApiKey("missing", authPath), null);
});
