...
test("derives the pairing port from the request host, then the startup flag", () => {
  // 反向代理换过端口时，浏览器实际访问的 Host 才作数。
  assert.equal(resolvePairPort("192.168.1.20:8443", "30141"), "8443");
  assert.equal(resolvePairPort("127.0.0.1:30141", undefined), "30141");
  assert.equal(resolvePairPort(null, " 5566 "), "5566");
  assert.equal(resolvePairPort(null, undefined), "30141");
  assert.equal(resolvePairPort("pi.local", ""), "30141");
});
