...
const { AgentEventConnection, AgentEventConnectionError, isAgentEventStreamAbort } = await jiti.import("./agent-event-connection.ts");
...
test("manual close rejects waiters and isolates late events", async () => {
  ...
  connection.close();
  await assert.rejects(abandoned, (error) => (
    error.status === "aborted"
    && isAgentEventStreamAbort(error)
...
test("a deliberate close neither retries nor reports an unexpected error", async () => {
  const { connection, sources, unexpectedErrors, setDemand } = createHarness();
  setDemand(true);
...
test("replacing the source for another session aborts the old waiter quietly", async () => {
  const { connection, sources, unexpectedErrors, setActiveSession, setDemand } = createHarness();
  setDemand(true);
  connection.maintain("session-a");
  setActiveSession("session-b");

  const ready = connection.ensureConnected("session-b");
  await wait(10);
...
test("transport failures are the only closed streams that surface as errors", () => {
  assert.equal(isAgentEventStreamAbort(new AgentEventConnectionError("closed")), false);
  assert.equal(isAgentEventStreamAbort(new AgentEventConnectionError("ready_timeout")), false);
  assert.equal(isAgentEventStreamAbort(new Error("nope")), false);
  assert.equal(isAgentEventStreamAbort(new AgentEventConnectionError("aborted")), true);
});
