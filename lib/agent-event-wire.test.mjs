...
test("drops transcript system messages from the stream", () => {
  // Pi >= 0.86 announces the prompt, tool loadout and section patches as system
  // messages. The leading one carries the whole prompt plus every tool schema, so
  // it must never reach the browser.
  const systemMessage = {
    role: "system",
    content: "",
    sections: { tools: "read, bash" },
    timestamp: 0,
  };

  assert.equal(isSystemMessageEvent({ type: "message_start", message: systemMessage }), true);
  ...
  // Ordinary messages and malformed events are untouched.
  assert.equal(isSystemMessageEvent({ type: "message_start", message: assistantMessage("") }), false);
  assert.equal(isSystemMessageEvent({ type: "message_start", message: "not-an-object" }), false);
  assert.equal(isSystemMessageEvent({ type: "message_update", message: systemMessage }), false);
  const userMessage = { type: "message_end", message: { role: "user", content: "hi" } };
  assert.strictEqual(toClientAgentEvent(userMessage), userMessage);
});
