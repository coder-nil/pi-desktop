...
test("reports a startup failure in-band after opening the transport", async (t) => {
  ...
  assert.equal(logged.some((line) => line.includes('"event":"agent_event_stream_start_failed"')), true);
  assert.equal(logged.some((line) => line.includes('"sessionId":"session-id"')), true);
  assert.equal(logged.some((line) => line.includes("broken config")), true);
});
