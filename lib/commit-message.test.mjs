...
test("previews a partially streamed message without failing on empty text", async () => {
  const { previewGeneratedCommitMessage } = await loadSubject();
  assert.equal(previewGeneratedCommitMessage("feat: add"), "feat: add");
  assert.equal(previewGeneratedCommitMessage("Commit message: feat: add"), "feat: add");
  assert.equal(previewGeneratedCommitMessage("   "), "");
  assert.equal(previewGeneratedCommitMessage("---"), "---");
  assert.doesNotThrow(() => previewGeneratedCommitMessage(""));
});
...
test("instructs the model to summarize only staged code changes", () => {
  ...
  assert.match(source, /streamSimple/);
  assert.match(source, /onUpdate\?: CommitMessageUpdate/);
});
