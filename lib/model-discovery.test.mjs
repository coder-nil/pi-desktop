...
test("preserves model capacities returned in metadata", () => {
...
test("ignores invalid discovered model capacities", () => {
  assert.deepEqual(parseDiscoveredModels({
    data: [{
      id: "invalid-capacities",
      metadata: { context_window: -1, max_output_tokens: "128000" },
    }],
  }), [{ id: "invalid-capacities" }]);
});
...
test("resolves environment-backed headers without an API key", async () => {
  ...
  try {
    const auth = await resolveModelDiscoveryAuth("pi-desktop-header-only-test", {
