...
test("never assigns the getter-only systemPrompt on agent state", async () => {
  // Pi >= 0.86 exposes agent.state.systemPrompt as a getter backed by the transcript's
  // leading system message, so assigning it throws at runtime while typecheck stays quiet
  // (AgentSessionLike is our own structural type). The language instruction reaches the
  // model through createLanguagePromptExtension instead; state stays read-only.
  const source = await readFile(new URL("./rpc-manager.ts", import.meta.url), "utf8");

  assert.doesNotMatch(source, /\.agent\.state\.systemPrompt\s*=(?!=)/);
  assert.match(source, /private effectiveSystemPrompt\(\): string/);
});

test("chat mode replaces the system prompt without clearing it and drops every tool", async () => {
  const source = await readFile(new URL("./rpc-manager.ts", import.meta.url), "utf8");
  const extensionSource = source.slice(
    source.indexOf("function createLanguagePromptExtension"),
    source.indexOf("const CODING_TOOL_NAMES"),
  );
  const startupSource = source.slice(source.indexOf("export async function startRpcSession"));
  ...
  const chatModeSource = source.slice(
    source.indexOf('case "set_chat_mode"'),
    source.indexOf('case "reload"'),
  );

  assert.match(source, /import \{ buildChatModeSystemPrompt \} from "\.\/chat-mode-prompt"/);
  // chatMode 优先于 forceEmpty：两者都不用工具，但只有 forceEmpty 会清空提示词。
  assert.match(extensionSource, /promptState\.chatMode\s*\?\s*buildChatModeSystemPrompt\(promptState\.value\)/);
  assert.match(source, /if \(this\.chatMode\) return buildChatModeSystemPrompt\(this\.uiLocale\);/);
  assert.match(stateSource, /chatMode: this\.chatMode,/);

  // 会话以普通对话模式创建时，调用方传入的工具集被忽略，提示词不被清空。
  assert.match(startupSource, /const toolNames = chatMode \? \[\] : options\.toolNames;/);
  assert.match(startupSource, /const forceEmpty = toolNames\?\.length === 0 && chatMode !== true;/);
  assert.match(startupSource, /if \(toolNames !== undefined && !chatMode\) \{/);
  assert.match(startupSource, /const workModeToolNames = chatMode \? inner\.getActiveToolNames\(\) : undefined;/);
  assert.match(startupSource, /wrapper\.setChatMode\(chatMode === true, workModeToolNames\)/);

  // 切换模式只改内存中的提示词来源与工具集，不重建 AgentSession。
  assert.match(chatModeSource, /this\.toolsBeforeChatMode = this\.inner\.getActiveToolNames\(\)/);
  assert.match(chatModeSource, /this\.inner\.setActiveToolsByName\(\[\]\)/);
  assert.match(chatModeSource, /toolNames: this\.inner\.getActiveToolNames\(\)/);
  assert.doesNotMatch(chatModeSource, /destroy\(/);
});
